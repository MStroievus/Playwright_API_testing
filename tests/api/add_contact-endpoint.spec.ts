...
import { test } from '../../app/fixture/auth_contact_api_client-fixture'
...
test.describe('Add Contact endpoint with valid data', async () => {
  test(
    ...
    async ({ contactAPIClient, builder }) => {
      await test.step('Build contact and send request', async () => {
  ...
  test(
    ...
    async ({ contactAPIClient, validation, builder }) => {
      await test.step('Build contact, send request and validate schemas', async () => {
  ...
  test(
    ...
    async ({ contactAPIClient, builder }) => {
      await test.step('Build contact without first name and send request', async () => {
  ...
  test(
    ...
    async ({ contactAPIClient, builder }) => {
      await test.step('Build contact without last name and send request', async () => {
  ...
  test(
    ...
    async ({ contactAPIClient, validation, builder }) => {
      await test.step('Build contact without first name, send request and validate response', async () => {
        const contact = builder.setLastName().build() as AddContactEndpoint
        const response = await contactAPIClient.addContact(contact);
        await validation.responseValidationSchema({
  ...
  test(
    `Check response with missing last name required field`,
    {
      tag: ['@api', '@regression']
    },
    async ({ contactAPIClient, validation, builder }) => {
      await test.step('Build contact without last name, send request and validate response', async () => {
        const contact = builder.setFirstName().build() as AddContactEndpoint
        const response = await contactAPIClient.addContact(contact);
        await validation.responseValidationSchema({
